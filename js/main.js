...
const makeRandomeElem = (arr) => arr[makeRandome(0, arr.length)];

//получаем массив уникальных ID
const makeRandomeId = (cb, length, max) => {
  const randomeIdArr = [];
  while (length > 0) {
    const curr = cb(0, max);
...
const idArr = makeRandomeId(makeRandome, COMMENTS_LENGTH, 200);

//шаблон для отдельного камента
const makeComments = (num) => ({
  id: num,
  avatar: `img/avatar-${makeRandome(1, COMMENTS_LENGTH)}.svg`,
  message: makeRandomeElem(COMMENTS_MESSAGE),
  name: makeRandomeElem(COMMENTS_NAME),
});
...
const randomeCommentsArr = () => {
  ...
  const randomeLength = makeRandome(1, COMMENTS_LENGTH);
...
const makeDescr = (num) => ({
  ...
  likes: makeRandome(MIN_LIKE, MAX_LIKE),
